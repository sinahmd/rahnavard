...
/**
 * Interactive image gallery with main viewer and thumbnail strip.
...
export default function CarImageGallery({ mainImage, gallery, persianName }: Props) {
  ...
  return (
    <>
      {/* Main image viewer */}
      <div
        className="relative w-full aspect-[4/3] bg-white rounded-[14px] overflow-hidden cursor-zoom-in group"
        onClick={() => setZoomed(true)}
      >
        <OptimizedImage
          src={activeImage}
          alt={`${persianName} - تصویر ${activeIndex + 1}`}
          width={800}
          height={600}
        ...
        {/* Image counter badge */}
        {allImages.length > 1 && (
          <div className="absolute bottom-3 left-3 bg-dark/70 text-white text-xs px-3 py-1.5 rounded-full backdrop-blur-sm font-medium">
        ...
        {allImages.length > 1 && (
          <>
            <button
              ...
              className="absolute top-1/2 right-3 -translate-y-1/2 w-10 h-10 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white transition-colors opacity-0 group-hover:opacity-100"
              aria-label="تصویر قبلی"
            >
              <svg className="w-5 h-5 text-dark rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M9 5l7 7-7 7" />
              </svg>
            </button>
            <button
              ...
              className="absolute top-1/2 left-3 -translate-y-1/2 w-10 h-10 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white transition-colors opacity-0 group-hover:opacity-100"
          ...
          {allImages.map((url, idx) => (
            <button
              ...
              className={`shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 transition-all duration-200 ${
                activeIndex === idx
                  ? 'border-accent-dark shadow-md scale-105'
      ...
      {zoomed && (
        ...
        >
          ...
          {/* Counter in lightbox */}
          {allImages.length > 1 && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-dark/70 text-white text-sm px-4 py-2 rounded-full backdrop-blur-sm">
              {activeIndex + 1} / {allImages.length}
            </div>
          )}
        </div>
