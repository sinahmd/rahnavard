...
**Status:** In progress — Phases 1–11 complete; Phase 6 cross-browser matrix + tmp-file cleanup pending
**Date:** 2026-09-10
...
### PHASE 10 — Why Rahnavard Section Background (managed upload)
- **Objective:** The section's background becomes a managed content image — admin-uploaded through the existing Site Settings UI and processed by the existing Phase 4A pipeline (ADR-0009) — replacing the hardcoded decorative SVG whenever an image is configured.
- **Scope:** One nullable `SiteSettings.why_background` ImageField (`upload_to="why/"`, content-image `ImageValidator` profile); additive serializer exposure (`why_background` + read-only `why_background_variants`); the three generic pipeline hooks only — `register_receiver`, `regenerate_image_variants.SOURCES`, `cleanup_orphan_media` referenced-set; one `ImageUpload` card in the existing admin settings form (mirrors the logo flow — replace-only, the form has no clear path); conditional layer in `WhyRahnavard.tsx` (`OptimizedImage fill` + white readability wash, decorative `alt=""`) with the SVG as the null fallback.
- **Out of scope (overengineering guardrail):** no `WhySection`/section-media model, no new endpoint or admin route, no slider/collection behavior, no Why-specific validator or variant generator. The settings payload grows <1 KB server-side (ISR-cached, RSC browsers never fetch it); a consolidated "Why section" admin tab remains possible later with zero backend change.
- **Tests:** admin multipart upload → variants generated after commit; public exposure incl. the `blur` data URL; `*_variants` null until generated; undersized upload → 400; regen source covers the field; replacement orphan classified; SVG fallback vs managed image vs degraded variants; section content unchanged.
- **Rollback:** revert the component condition (SVG returns), the serializer fields, and the receiver/registry lines; migration is additive (drop column).
- **Dependencies:** Phase 4A pipeline (ADR-0009); independent of the other post-4B phases and of Phase 6's pending follow-ups.

### PHASE 11 — Admin Panel Responsive Redesign (added 2026-09-21)
- **Objective:** The authenticated admin SPA is genuinely usable from a phone: navigation, tables, forms, dialogs, and upload controls work at 320–430px without horizontal scrolling or hover-only affordances, while desktop behavior stays unchanged.
- **Scope (frontend admin surface only):** responsive shell in `admin/(protected)/layout.tsx` — below `lg` the navigation is a focus-trapped overlay drawer (shared `NavLinks`/`UserCard`/`SidebarFooter` with the desktop sidebar; `useModalA11y`, labeled close control, body scroll lock, `aria-controls`/`aria-hidden` state), at `lg+` the classic collapsible sidebar with the toggle in the topbar; `AdminListPage` renders rows as stacked cards below `md` driven by two declarative column flags (`hideOnMobile`, `primaryOnMobile` — the primary column renders as the card's title line, first, regardless of desktop column order); `ConfirmDialog` becomes a bottom sheet on phones with stacked full-width buttons (destructive last) and safe-area padding; `AdminForm`/settings get a shared sticky mobile action bar (`components/admin/ui/FormActions.tsx`); `PageHeader` wraps its action below the title; hover-only upload affordances become always-visible touch controls with 44px/36px tap targets; row-action buttons normalize to a 36px secondary-control scale.
- **Out of scope:** desktop table column order, the dual table+cards rendering strategy (kept — one DOM, CSS-gated), the settings page's hand-rolled form shape (documented Phase 4 deviation), all data flow — `AdminListPage` fetch/pagination logic, `useAdminForm`, `lib/api` transport, session guard, and routing are untouched.
- **Tests:** existing suites adapted for intentional dual rendering (`getAllBy*`); new pins for the mobile-card/table split, primary-first card ordering, and the drawer's `aria-controls`/`aria-hidden`/labeled-close contract. Full jest suite green, `tsc` clean, `lint` clean (2026-09-21: **402 passed / 51 suites**).
- **Rollback:** revert the working-tree diff — pure presentation change, no migrations, no API or auth surface.
- **Dependencies:** builds on the Phase 3–5 admin primitives (`AdminListPage`, `AdminForm` split, `PageHeader`/`ConfirmDialog`/`useModalA11y`); independent of Phase 6's pending follow-ups.
...
**Phase 10 (added 2026-09-20):** one managed `SiteSettings.why_background` field riding the Phase 4A pipeline — backend + frontend, but no new architectural surface; independent of Phases 5–9 and of Phase 6's pending follow-ups.
**Phase 11 (added 2026-09-21):** frontend-only responsive redesign of the admin surface, layered on the Phase 3–5 admin primitives; independent of Phase 6's pending follow-ups.
...
| 2026-09-21 | **Phase 11 (Admin Panel Responsive Redesign) implemented** — responsive shell (mobile drawer <lg + collapsible desktop sidebar) in `admin/(protected)/layout.tsx` with `aria-controls`/`aria-hidden` and the shared `useModalA11y` focus contract; `AdminListPage` mobile card list driven by `hideOnMobile`/`primaryOnMobile` column flags (primary renders first); `ConfirmDialog` bottom sheet on phones; shared sticky `FormActions` bar (`components/admin/ui/FormActions.tsx`) for `AdminForm` + settings; hover-only upload affordances replaced with always-visible touch controls; row actions normalized to a 36px tap scale; RTL fixes (`text-start`, `dir="ltr"` phone values) and truncated `AdminForm` heading. Desktop layouts unchanged; data flow, API client, auth guard and routing untouched. Two defects caught by the temporary Playwright width matrix (`e2e/tmp-admin-responsive.spec.ts`, gitignored) and fixed during verification: (1) the drawer panel used `transition-all`, animating `visibility` so the close button was unfocusable for the first ~150 ms — initial focus silently no-op'd; switched to `transition-transform`; (2) `globals.css` set `overflow-x: hidden` on both `html` and `body`, making `body` a non-scrolling container so `position: sticky` descendants (the FormActions bars) never traveled; the clip now lives on `html` only. Verified in Docker: jest **402 passed / 51 suites**, `tsc` clean, `lint` clean, width matrix green at 320/360/390/430/768/1024/1280 + RTL (project config: `workers=1, retries=1` against the dev stack). Senior review of the working-tree diff (2026-09-21) approved it as a presentation-only change. |
| 2026-09-20 | **Phase 10 (Why Rahnavard section background) complete** — one nullable `SiteSettings.why_background` field (`why/`, content-image profile) joins the existing Phase 4A pipeline through the generic hooks only: upload-time receiver, `regenerate_image_variants.SOURCES`, `cleanup_orphan_media` referenced-set, additive serializer `why_background`/`why_background_variants`. Admins manage it from the existing Site Settings form via the shared `ImageUpload` (mirrors the logo flow — replace-only; the form has no clear path, documented). `WhyRahnavard` renders the image (lg WebP tier, LQIP blur, decorative `alt=""`) under a white readability wash and keeps the hardcoded SVG when nothing is configured. Explicitly rejected per the guardrail: a WhySection model, new endpoint/route, slider behavior — HeroSlide's mechanics were reused without its domain architecture. A consolidated Why-section admin tab remains possible later with zero backend change. Verified in local Docker: backend **379 passed / 98.28% cov**, `check` + `makemigrations --check` clean; frontend **387 passed / 50 suites**, tsc clean, lint clean. Incident recorded: the first `apps.py` edit truncated mid-write (receiver call lost — variants silently absent); caught by the new tests and restored, pinned by `TestWhyBackgroundAdminUpload`. **Same-day follow-up — the consolidated Why-section admin tab (admin UI only, zero backend change):** new `/admin/why-rahnavard` tab grouping the section copy + background (a *partial* settings PATCH carrying only the why fields) with the embedded feature list (`AdminListPage` reuse); feature create/edit routes moved to `/admin/why-rahnavard/features/…`, the old list page removed, sidebar «ویژگی‌ها» replaced by «بخش چرا راهنورد», and the why fields removed from the general settings form (field-ownership split — the settings PATCH no longer touches why fields, so no cross-tab stale overwrites). Verified: frontend **390 passed / 51 suites**, tsc clean, lint clean; backend untouched. |
