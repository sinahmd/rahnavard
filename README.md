...
The refactor plan is fully implemented except two deliberately open gates: the CSP header is still **report-only** (flip to enforcing after a violation-free review window) and the Phase 2 token cutover remains gated on the staging smoke checklist and owner approval. Minimal Playwright E2E smoke specs live in [`frontend/e2e/`](./frontend/e2e/README.md) and run against the local Docker stack.
